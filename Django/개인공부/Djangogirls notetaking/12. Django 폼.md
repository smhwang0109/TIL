...
브라우저에 사이트를 다시 불러오면 `AttributeError`가 보이게 됩니다. 왜냐하면, 아직 `post_new`뷰를 구현하지 않았기 때문이죠. 이제 하나 더 추가해봅시다.
...
- `Save` 버튼이 필요합니다. 이것은 HTML 버튼으로 만들 수 있어요: `Save`
- 마지막으로 ``을 열어 `{% csrf_token %}`를 추가하세요. 이 작업은 폼 보안을 위해 중요하답니다! 이 작업을 빼먹고 저장하면 장고는 이렇게 불평할 거에요.
...
다 작성했으면 화면을 다시 불러옵시다. 폼이 이렇게 나타났나요!

![New form](https://tutorial.djangogirls.org/ko/django_forms/images/new_form2.png)
...
![Logged in error](https://tutorial.djangogirls.org/ko/django_forms/images/post_create_error.png)
...
![Form validation](https://tutorial.djangogirls.org/ko/django_forms/images/form_validation2.png)
...
![Edit button](https://tutorial.djangogirls.org/ko/django_forms/images/edit_button2.png)

수정 버튼을 누르면 우리가 쓴 블로그 글이 들어가 있는 폼을 볼 수 있을 거에요.

![Edit form](https://tutorial.djangogirls.org/ko/django_forms/images/edit_form2.png)
...
여기에 `{% if %}`태그를 추가해 관리자로 로그인한 유저들만 링크가 보일 수 있게 만들 거에요. 그게, 바로 여러분이죠! ``태그를 아래와 같이 변경하세요.
...
이 `{% if %}`는 브라우저에 페이지를 요청 하는 사용자가 로그인 하는 경우 링크가 발생됩니다. 이는 새 게시글을 완전히 보호해주는 것은 아니지만, 바람직한 방법입니다. 이 부분은 장고걸스 심화 튜토리얼에서 좀더 자세히 다룰거에요.
