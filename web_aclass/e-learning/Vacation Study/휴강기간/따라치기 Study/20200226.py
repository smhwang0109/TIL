...
지급일이 주말이면서 다음 평일이 익월일 경우의 예외 반영
오늘이 12월일 경우 예외 반영
...
def count(payday):
    ...
    if date.today() > target:  # 다음달 급여일
        try:
            target = date(date.today().year, date.today().month+1, payday)
        except ValueError:
            target = date(date.today().year+1, 1, payday)  # 12월 예외

            
        while target.weekday() in [5,6]:
            target = target + timedelta(days=1)

    return int((target-date.today())/timedelta(days=1)) # timedelta 자료형을 일단위 정수형으로 보정


if __name__ == '__main__':
    for i in range(1, 29):
        print(f'예정일 : {i}, 남은 일수 : {count(i)}')
